...
pub fn wasm_const_impl(item: ItemConst, _attr: Nothing) -> Result<TokenStream> {
    ...
    let const_value = match &*item.expr {
        Expr::Lit(syn::ExprLit { lit, .. }) => match lit {
            ...
            _ => return Err(syn::Error::new_spanned(&item.expr, "Unsupported literal type")),
        },
        Expr::Array(array) => {
            let elements: Result<Vec<_>> = array
                ...
                .map(|elem| match elem {
                    Expr::Lit(syn::ExprLit { lit, .. }) => match lit {
                        ...
                        Lit::Bool(value) => Ok(quote! { #value }),
                        Lit::Str(value) => Ok(quote! { #value }),
                        _ => Err(syn::Error::new_spanned(elem, "Unsupported array element type")),
                    },
                    _ => Err(syn::Error::new_spanned(elem, "Expected a literal type in array")),
                })
                .collect();
        ...
        Expr::Unary(ExprUnary { op: UnOp::Neg(_), expr, .. }) => match &**expr {
            ...
            _ => return Err(syn::Error::new_spanned(&item.expr, "Expected a literal after unary operator")),
        },
        _ => return Err(syn::Error::new_spanned(&item.expr, "Expected a literal or array")),
    }?;
