...
mod tests {
    ...
    fn test_sequence_with_one_tick_array(tick_spacing: u16, ticks: [TickFacade; TICK_ARRAY_SIZE], start_tick_index: i32) -> TickArraySequenceVec {
        let one = TickArrayFacade { start_tick_index, ticks };
        TickArraySequenceVec::new(vec![one], tick_spacing).unwrap()
    }

    fn test_sequence(tick_spacing: u16, ticks: [TickFacade; TICK_ARRAY_SIZE]) -> TickArraySequenceVec {
        ...
        TickArraySequenceVec::new(vec![one, two, three], tick_spacing).unwrap()
    }
